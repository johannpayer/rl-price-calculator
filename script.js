...
function update() {
    ...
    let safeUnpaintedValue = unpaintedValue.value === '0' || onlyPaintedDrop ? typeValues[itemType.selectedIndex] || baseValue.value : unpaintedValue.value;
    let colorMultiplier = Math.max(Math.pow(baseValue.value / safeUnpaintedValue, 0.182) - 1, 0);
    if (onlyPaintedDrop) {
